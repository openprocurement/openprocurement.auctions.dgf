...
class AuctionCancellationResource(APIResource):
    ...
    def cancel_lot(self, cancellation=None):
        ...
        auction = self.request.validated['auction']
        [setattr(i, 'status', 'cancelled')
         for i in auction.lots if i.id == cancellation.relatedLot]
    ...
    def collection_post(self):
        ...
        if auction.status in ['complete', 'cancelled', 'unsuccessful']:
            self.request.errors.add(
                'body',
                'data',
                'Can\'t add cancellation in current ({}) auction status'.format(
                    auction.status))
            self.request.errors.status = 403
            return
        cancellation = self.request.validated['cancellation']
        cancellation.date = get_now()
        if any([i.status != 'active' for i in auction.lots if i.id ==
                cancellation.relatedLot]):
            self.request.errors.add(
                'body', 'data', 'Can add cancellation only in active lot status')
            self.request.errors.status = 403
            return
        ...
        if save_auction(self.request):
            self.LOGGER.info(
                'Created auction cancellation {}'.format(
                    cancellation.id), extra=context_unpack(
                    self.request, {
            ...
            self.request.response.headers['Location'] = self.request.current_route_url(
                _route_name=route, cancellation_id=cancellation.id, _query={})
    ...
    def collection_get(self):
        ...
        return {
            'data': [
                i.serialize("view") for i in self.request.validated['auction'].cancellations]}
    ...
    def get(self):
        ...
        """
        return {
            'data': self.request.validated['cancellation'].serialize("view")}
    ...
    def patch(self):
        ...
        if auction.status in ['complete', 'cancelled', 'unsuccessful']:
            self.request.errors.add(
                'body',
                'data',
                'Can\'t update cancellation in current ({}) auction status'.format(
                    auction.status))
            self.request.errors.status = 403
            return
        if any([i.status != 'active' for i in auction.lots if i.id ==
                self.request.context.relatedLot]):
            self.request.errors.add(
                'body', 'data', 'Can update cancellation only in active lot status')
            self.request.errors.status = 403
            return
        apply_patch(
            self.request,
        ...
        if save_auction(self.request):
            self.LOGGER.info(
                'Updated auction cancellation {}'.format(
