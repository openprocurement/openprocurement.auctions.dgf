...
class AuctionLotResource(APIResource):
    ...
    def collection_post(self):
        ...
        if auction.status not in ['active.tendering']:
            self.request.errors.add(
                'body',
                'data',
                'Can\'t add lot in current ({}) auction status'.format(
                    auction.status))
        ...
        if save_auction(self.request):
            self.LOGGER.info(
                'Created auction lot {}'.format(
                    lot.id), extra=context_unpack(
                    self.request, {
                        'MESSAGE_ID': 'auction_lot_create'}, {
                        'lot_id': lot.id}))
    ...
    def collection_get(self):
        ...
        return {
            'data': [
                i.serialize("view") for i in self.request.validated['auction'].lots]}
    ...
    def patch(self):
        ...
        if auction.status not in ['active.tendering']:
            self.request.errors.add(
                'body',
                'data',
                'Can\'t update lot in current ({}) auction status'.format(
                    auction.status))
            self.request.errors.status = 403
            return
        if apply_patch(self.request, src=self.request.context.serialize()):
            self.LOGGER.info(
                'Updated auction lot {}'.format(
                    self.request.context.id), extra=context_unpack(
                    self.request, {
                        'MESSAGE_ID': 'auction_lot_patch'}))
            return {'data': self.request.context.serialize("view")}
    ...
    def delete(self):
        ...
        if auction.status not in ['active.tendering']:
            self.request.errors.add(
                'body',
                'data',
                'Can\'t delete lot in current ({}) auction status'.format(
                    auction.status))
            self.request.errors.status = 403
            return
        ...
        if save_auction(self.request):
            self.LOGGER.info(
                'Deleted auction lot {}'.format(
                    self.request.context.id), extra=context_unpack(
                    self.request, {
                        'MESSAGE_ID': 'auction_lot_delete'}))
            return {'data': res}
