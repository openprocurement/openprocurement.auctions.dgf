...
class AuctionQuestionResource(APIResource):
    ...
    def collection_post(self):
        ...
        ) < auction.enquiryPeriod.startDate or get_now() > auction.enquiryPeriod.endDate:
            self.request.errors.add(
                'body', 'data', 'Can add question only in enquiryPeriod')
        ...
        if save_auction(self.request):
            self.LOGGER.info(
                'Created auction question {}'.format(
                    question.id), extra=context_unpack(
                    self.request, {
    ...
    def collection_get(self):
        """List questions
        """
        return {'data': [i.serialize(self.request.validated['auction'].status)
                         for i in self.request.validated['auction'].questions]}

    @json_view(permission='view_auction')
    def get(self):
        """Retrieving the question
        """
        return {'data': self.request.validated['question'].serialize(
    ...
    def patch(self):
        ...
        if auction.status != 'active.tendering':
            ...
            return
        if any([i.status != 'active' for i in auction.lots if i.id ==
                self.request.context.relatedItem]):
            self.request.errors.add(
                'body', 'data', 'Can update question only in active lot status')
            self.request.errors.status = 403
            return
        if apply_patch(self.request, src=self.request.context.serialize()):
            self.LOGGER.info(
                'Updated auction question {}'.format(
