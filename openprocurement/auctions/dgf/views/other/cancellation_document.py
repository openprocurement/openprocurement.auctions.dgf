...
@opresource(
    name='dgfOtherAssets:Auction Cancellation Documents',
...
class AuctionCancellationDocumentResource(APIResource):
    ...
    def collection_post(self):
        ...
        if save_auction(self.request):
            self.LOGGER.info(
                'Created auction cancellation document {}'.format(
    ...
    def put(self):
        ...
        if save_auction(self.request):
            self.LOGGER.info(
                'Updated auction cancellation document {}'.format(
                    self.request.context.id), extra=context_unpack(
                    self.request, {
                        'MESSAGE_ID': 'auction_cancellation_document_put'}))
            return {'data': document.serialize("view")}
        ...
        if apply_patch(self.request, src=self.request.context.serialize()):
            ...
            self.LOGGER.info(
                'Updated auction cancellation document {}'.format(
                    self.request.context.id), extra=context_unpack(
                    self.request, {
                        'MESSAGE_ID': 'auction_cancellation_document_patch'}))
