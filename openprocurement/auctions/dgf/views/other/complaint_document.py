...
@opresource(
    name='dgfOtherAssets:Auction Complaint Documents',
...
class AuctionComplaintDocumentResource(APIResource):
    ...
    def collection_post(self):
        ...
        if save_auction(self.request):
            self.LOGGER.info(
                'Created auction complaint document {}'.format(
                    document.id), extra=context_unpack(
                    self.request, {
                        'MESSAGE_ID': 'auction_complaint_document_create'}, {
    ...
    def put(self):
        ...
        if self.request.authenticated_role != self.context.author:
            self.request.errors.add(
                'url', 'role', 'Can update document only author')
            self.request.errors.status = 403
            return
        if self.request.validated['auction_status'] not in [
            'active.tendering',
        ...
        if save_auction(self.request):
            self.LOGGER.info(
                'Updated auction complaint document {}'.format(
                    ...
                    self.request, {
                        'MESSAGE_ID': 'auction_complaint_document_put'}))
            return {'data': document.serialize("view")}
