...
from openprocurement.auctions.dgf.tests.base import test_auction_data
# AuctionLotResourceTest


def create_auction_lot_invalid(self):
    response = self.app.post_json('/auctions/some_id/lots',
...
def create_auction_lot(self):
    ...
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
        {u'description': [u'lot guarantee currency should be identical to auction guarantee currency'],
    ...
    self.assertEqual(response.json['errors'][0]["description"],
                     "Can't add lot in current (active.tendering) auction status")
...
def patch_auction_lot(self):
    ...
    response = self.app.patch_json('/auctions/{}/lots/{}'.format(self.auction_id, lot['id']),
    ...
    response = self.app.patch_json('/auctions/{}/lots/some_id'.format(self.auction_id),
    ...
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['errors'][0]["description"],
                     "Can't update lot in current (active.tendering) auction status")
...
def patch_auction_currency(self):
    ...
    response = self.app.patch_json('/auctions/{}'.format(self.auction_id), {"data": {"value": {"currency": "GBP"}}},
                                   status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
...
def delete_auction_lot(self):
    ...
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['errors'][0]["description"],
                     "Can't delete lot in current (active.tendering) auction status")
...
def auction_lot_guarantee(self):
    ...
    response = self.app.patch_json('/auctions/{}/lots/{}'.format(auction['id'], lot2_id),
    ...
    for l_id in (lot_id, lot2_id):
        response = self.app.patch_json('/auctions/{}/lots/{}'.format(auction['id'], l_id),
...
def auction_value(self):
    ...
    self.assertEqual(response.json['data']['value']['amount'],
                     sum([i['value']['amount'] for i in self.initial_lots]))
    self.assertEqual(response.json['data']['minimalStep']['amount'],
                     min([i['minimalStep']['amount'] for i in self.initial_lots]))
...
def create_auction_bidder_invalid(self):
    ...
    response = self.app.post_json(request_path, {'data': {'tenderers': [self.initial_organization], 'lotValues': [
        {"value": {"amount": 50}, 'relatedLot': self.initial_lots[0]['id']}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.post_json(request_path, {
        'data': {'tenderers': [self.initial_organization], "value": {"amount": 500},
                 'lotValues': [{"value": {"amount": 500}, 'relatedLot': self.initial_lots[0]['id']}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
